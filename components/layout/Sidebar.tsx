...
export default function MainSidebar({ activeTab, setActiveTab, onClose }:{
  activeTab: DashboardTab; setActiveTab:(t:DashboardTab)=>void; onClose?:()=>void;
}) {
  return (
    <aside className="h-[100svh] w-64 sm:w-64 shrink-0 flex flex-col
                      bg-[#0e1a2b]/90 backdrop-blur-md border-r border-white/10 text-[#dbe7ff]">
      <div className="flex justify-between items-center px-4 py-5">
        <img src="/logo.png" alt="BetaOffice" className="h-9 w-auto max-w-[160px]" />
        {onClose && (
          <button onClick={onClose} className="md:hidden text-[#c5d6ff]/70 hover:text-white transition" aria-label="Close menu">✕</button>
        )}
      </div>

      <nav className="px-2 sm:px-3 py-2 space-y-1 flex-1">
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                "touch-manipulation",
                active ? "bg-white/8 text-white shadow-inner ring-1 ring-inset ring-white/10"
            ...
            >
              ...
              {active && (<span className="absolute right-1 top-1/2 -translate-y-1/2 h-5 w-5 rounded-full blur-md opacity-40 pointer-events-none bg-gradient-to-br from-fuchsia-500 to-blue-500" />)}
